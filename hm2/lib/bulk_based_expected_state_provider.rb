...
module HealthManager2
  ...
  class BulkBasedExpectedStateProvider < ExpectedStateProvider
    ...
    def set_expected_state(known, expected)
      known.num_instances = ['instances']
      known.state = expected['state']
      known.live_version = expected['version']
      known.framework = expected['staging']['model']
      known.runtime = expected['staging']['stack']
      #known.last_updated = ???? ensure bulk_api returns the damn thing
    end
