...
TEST_REMOTE_PIP = 'fluentxml-0.1.1.tar.bz2'
...
def fetch_remote_gem(gem_name)
  url = GemUtil.gem_to_url(gem_name)
...
begin
    ...
    puts "adding remote pip #{TEST_REMOTE_PIP} to cache"
    client.add_package(:remote, :pip, TEST_REMOTE_PIP)
    check_pkg_in_cache(client, TEST_REMOTE_PIP, :remote)
ensure
  exit_test_root
end
