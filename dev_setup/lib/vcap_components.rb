...
class Component
  ...
  def self.register(name, excluded=false)
...
class ServiceComponent < Component
  %w(redis mysql mongodb rabbitmq postgresql vblob).each do |service|
    register("#{service}_gateway")
    register("#{service}_node")
  end

  register("filesystem_gateway")

  # excluded services
  %w(neo4j memcached couchdb elasticsearch).each do |service|
    register("#{service}_gateway", true)
    register("#{service}_node", true)
  end
