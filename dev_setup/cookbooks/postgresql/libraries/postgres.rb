module CloudFoundryPostgres
  def cf_pg_update_hba_conf(db, user)
    case node['platform']
    when "ubuntu"
      ruby_block "Update postgres config" do
        block do
          ...
          `/etc/init.d/postgresql-#{pg_major_version} restart`
        end
      end
    else
      Chef::Log.error("Postgres config update is not supported on this platform.")
    end
  end
  ...
  def cf_pg_setup_db(db, user, passwd)
    case node['platform']
    when "ubuntu"
      bash "Setup postgres database #{db}" do
        user "postgres"
        code <<-EOH
        createdb #{db}
        psql -d #{db} -c \"create role #{user} NOSUPERUSER LOGIN INHERIT CREATEDB\"
        psql -d #{db} -c \"alter role #{user} with password '#{passwd}'\"
        echo \"db #{db} user #{user} pass #{passwd}\" >> /tmp/cf_pg_setup_db
        EOH
      end
    else
      Chef::Log.error("Postgres database setup is not supported on this platform.")
    end
  end
end
