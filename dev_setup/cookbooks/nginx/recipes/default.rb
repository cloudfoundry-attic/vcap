...
%w{build-essential zlib1g zlib1g-dev libssl-dev}.each do |pkg|
  package pkg
end
...
# Assumes that Ubuntu has set user and group name to be the same
user = group = node[:deployment][:user]
...
# nginx is used in the router and cloud controller with two different configuration files
#
# For now, Chef will manually start nginx as our configuration is non-standard
# and follows how BOSH packages are installed

bash "start_nginx" do
  cwd File.join(node[:nginx][:prefix])
  code <<-EOH
    sudo #{node[:nginx][:prefix]}/sbin/nginx -c #{node[:nginx][:prefix]}/conf/router_nginx.conf
    sudo #{node[:nginx][:prefix]}/sbin/nginx -c #{node[:nginx][:prefix]}/conf/cloud_controller_nginx.conf
  EOH
  creates node[:nginx][:started]
end
