...
module DEA
  ...
  class Agent
    ...
    def spawn(cmd, opts, cb = nil)
      ...
      env.each do |k, v|
        w.puts("export %s=%s" % [k, v]) rescue SystemCallError
      end

      # exec
      w.puts("exec %s" % [cmd]) rescue SystemCallError

      # Close pipe and don't care if this fails
      r.close rescue SystemCallError
      w.close rescue SystemCallError
