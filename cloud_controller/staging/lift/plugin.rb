...
class LiftPlugin < StagingPlugin
  ...
  def stage_application
    Dir.chdir(destination_directory) do
      ...
      configure_cf_lift_filter(webapp_root)
...
ruby resources/generate_server_xml $PORT
  ...
  # if we find a LiftFilter node, we add a new servlet filter configuration
  # *before* the LiftFilter configuration.
  # The added servlet filter is responsible for generating a properties
  # file that is consulted by the Lift framework to determine the binding
  # information of the services used by the application.
  # Both components of the new filter configuration ('filter' and
  # 'filter-mapping') are generated and added before the corresponding
  # counterparts of the existing LiftFilter.
  def configure_cf_lift_filter(webapp_path)
    ...
    if File.exist? web_config_file
      ...
      unless lift_filter == nil || lift_filter.empty?
        ...
        lift_filter_map.first.add_previous_sibling(
            cf_lift_props_generator_filter_map)
