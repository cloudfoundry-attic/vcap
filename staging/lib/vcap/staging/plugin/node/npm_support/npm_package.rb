...
class NpmPackage
  ...
  def install
    if url_provided?
      @logger.info("Failed installing package #{@name}. URLs are not supported")
      return nil
    end
    ...
    unless resolved.is_a?(Hash) && resolved["version"]
      ...
      @logger.info("Failed getting the requested package: #{log_name}")
      return nil
    end
  ...
  def safe_install
    ...
    if child_status != 0
      @logger.info("Failed installing package: #{@name}")
  ...
  def resolved_version_data
    ...
    if $?.exitstatus != 0 or output.empty?
      return nil
    else
      begin
