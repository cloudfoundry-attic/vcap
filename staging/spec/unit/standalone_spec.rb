...
wait $STARTED
  ...
  describe "with no runtime" do
    before do
      app_fixture :standalone_script
    end
    it "is packaged with a startup script" do
      stage(:standalone,{:meta=>{:command=> "hello.sh"}, :runtime=> nil}) do |staged_dir|
          start_script = File.join(staged_dir, 'startup')
          start_script.should be_executable_file
          script_body = File.read(start_script)
          script_body.should == <<-EXPECTED
#!/bin/bash
cd app
hello.sh > ../logs/stdout.log 2> ../logs/stderr.log &
STARTED=$!
echo "$STARTED" >> ../run.pid
wait $STARTED
          EXPECTED
    ...
    it "is able to execute provided script" do
      stage(:standalone,{:meta=>{:command=> "hello.sh"}, :runtime=> nil}) do |staged_dir|
        user_script = File.join(staged_dir, 'app','hello.sh')
        user_script.should be_executable_file
      end
    end
  end
  describe "with Python runtime" do
    ...
    it "is packaged with a startup script" do
      stage(:standalone,{:meta=>{:command=> "python HelloWorld.py"}, :runtime=> "python26", :environment=>{:resources=>{:memory=>512}}}) do |staged_dir|
          start_script = File.join(staged_dir, 'startup')
          start_script.should be_executable_file
          script_body = File.read(start_script)
          script_body.should == <<-EXPECTED
