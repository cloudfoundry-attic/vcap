...
class VCAP::Stager::ProcessRunner
  ...
  # Runs the supplied command, logs the result, and raises an exception if
  # the command exits with an unexpected status.
  #
  # @option opts [Integer]  :expected_exitstatus
  #
  # @see VCAP::Stager::ProcessRunner#run
  def run_logged!(cmd, opts = {})
    expected_exitstatus = opts[:expected_exitstatus] || 0

    ret = run_logged(cmd, opts)

    unless ret[:status].exitstatus == expected_exitstatus
      emsg = "Command '#{cmd}' exited with unexpected status" \
             + " #{ret[:status].exitstatus}"
      raise VCAP::Stager::ProcessRunnerError.new(emsg)
    end
