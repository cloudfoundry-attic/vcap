...
class VCAP::Stager::Task
  ...
  def initialize(app_id, props, download_uri, upload_uri, notify_subj, opts={})
    ...
    @download_app_path    = option(opts, :download_app_path)
    @upload_app_path      = option(opts, :upload_app_path)
  ...
  def download_app(dst_dir, tmp_dir)
    ...
    cmd = "#{@download_app_path} #{uri_path} #{zipped_app_path}"
    res = run_logged(cmd)
    unless res[:success]
  ...
  def upload_droplet(staged_dir, tmp_dir)
    ...
    cmd = "#{@upload_app_path} #{uri_path} #{droplet_path}"
    res = run_logged(cmd)
    unless res[:success]
